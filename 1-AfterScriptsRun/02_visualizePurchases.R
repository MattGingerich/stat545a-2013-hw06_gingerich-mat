...
print(p)
ggsave(path=file.path(figureFolder, "01-LargestOrders.png"))
...
print(p)
ggsave(path=file.path(figureFolder, "02-EarningsBySupplier.png"))
...
p <- p + scale_y_log10(labels = trans_format("log10", math_format(10^.x)))
p <- p + coord_flip()
p <- p + ggtitle(paste("The", n, "Largest Suppliers by Total Earnings in 2010-2011"))
print(p)
ggsave(path=file.path(figureFolder, "03-BiggestSuppliers.png"))
...
print(p)
ggsave(path=file.path(figureFolder, "04-PriciestAreas.png"))